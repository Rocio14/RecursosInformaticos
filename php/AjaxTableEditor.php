...
class AjaxTableEditor {
    ...
    public function setDefaults() {
        ...
        $this->defNumAdvSearches = isset($this->defNumAdvSearches) ? $this->defNumAdvSearches : 1;
...
//    public function clear_adv_search() {
//        $delCriteria = $this->info['delCriteria'];
//        unset($this->info['delCriteria']);
//        $this->sessionData['start'] = 0;
//        foreach ($this->info as $i => $info) {
//            $this->clear_adv_search[$i] = $this->info[$i];
//        }
//        $this->sessionData['clear_adv_search'] = $this->clear_adv_search;
//        if ($delCriteria) {
//            $this->sessionData['numAdvSearches'] ++;
//            $this->numAdvSearches ++;
//        }
...
//    }
    ...
    public function formatDate($date, $format) {
        if (isset($this->langVars->language) && $this->langVars->language != 'Spanish') {
            $spanishMonthNames = $this->getMonthNamesArr();
            $otherMonthNames = $this->getMonthNamesArr($this->langVars->locales);
            $date = str_ireplace($otherMonthNames['long'], $spanishMonthNames['long'], $date);
            $date = str_ireplace($otherMonthNames['short'], $spanishMonthNames['short'], $date);
        }
        $ts = strtotime($date);
        if ($ts !== false && $ts > 0) {
    ...
    public function getMonthNamesArr($locales = array('es_ES.utf8', 'es_MX.utf8', 'es_ES', 'es_MX', 'es', 'spanish')) {
        $monthNames = array();
...
//        return $html;
    ...
    public function setLangVars() {
        if (class_exists('LangVars')) {
            ...
            $langVarsPath = str_replace('AjaxTableEditor.php', 'lang/LangVars-es.php', __file__);
