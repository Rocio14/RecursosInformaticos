...
//	}
    ...
    protected function initiateEditor() {
        $tableColumns['id_contacto'] = array(
            'display_text' => 'Id',
            'req' => true,
            'perms' => 'XQSO'
        );
        ...
        $tableColumns['num_empleado'] = array(
            'display_text' => '# Empleado',
            'req' => true,
            'perms' => 'EVCAXQSHO',
            'val_fun' => array(&$this,'valnum')
        );

        $tableColumns['nombre_con'] = array(
            'display_text' => 'Empleado',
            'perms' => 'EVCTAXQSHO',
            'req' => true,
        ...
        $tableColumns['tel_directo'] = array(
            'display_text' => 'Tel_Directo',
            'req' => true,
            'perms' => 'EVCTAXQSHO',
            'val_fun' => array(&$this,'valte')
        );

        $tableColumns['tel_ext'] = array(
            'display_text' => 'Ext',
            'req' => true,
            'perms' => 'EVCTAXQSHO',
            'val_fun' => array(&$this,'valnum')
        ...
        $tableColumns['tel_celular'] = array(
            'display_text' => 'Cel.',
            'req' => true,
            'perms' => 'EVCTAXQSHO',
            'val_fun' => array(&$this,'valte')
        ...
        $tableColumns['correo1'] = array(
            'display_text' => 'Correo Institucional',
            'req' => true,
            'perms' => 'EVCTAXQSHO',
             'val_fun' => array(&$this,'valEmail')
        );

        $tableColumns['correo2'] = array(
            'display_text' => 'Correo Alternativo',
            'req' => true,
            'perms' => 'EVCTAXQSHO',
             'val_fun' => array(&$this,'valEmail')
        );
...
//            'perms' => 'EVCTAXQSHO'
//        );
        $tableColumns['fecha_alta'] = array(
            'display_text' => 'Fecha de alta',
            'perms' => 'EATVXSQ',
            'display_mask' => 'date_format(`fecha_alta`,"%d %M %Y")',
            'order_mask' => 'date_format(`fecha_alta`,"%Y-%m-%d %T")',
            'range_mask' => 'date_format(`fecha_alta`,"%Y-%m-%d %T")',
          'calendar' => array(
               'js_format' => 'yymmdd',
              'options' => array('showButtonPanel' => true))
        );
	$tableColumns['activo'] = array(
            'display_text' => 'Activo',
            'req' => true,
            'perms' => 'EVCAXQSFHOT',
            'select_array' => array(
        ...
        $this->Editor->setConfig('tableTitle', 'Contacto');
        $this->Editor->setConfig('addRowTitle', 'Agregar Contacto');
        $this->Editor->setConfig('editRowTitle', 'Editar Contacto');
    ...
    function valEmail($col,$val,$row,$instanceName)
{
     ...
     else
     {
          // Create custom validation message and return false
          $this->Editor->showDefaultValidationMsg = false;
          $this->Editor->addTooltipMsg('Porfavor ingrese su e-mail');
          return false;
     }
}
...
function valte($col,$valte,$row,$instanceName)
{
     if (preg_match('/^\d{2}-\d{2}-\d{2}-\d{2}$/' , $valte)|| preg_match('/^\d{2}-\d{2}-\d{2}-\d{2}$/' , $valte))
     {
          return true;
     }
     else
